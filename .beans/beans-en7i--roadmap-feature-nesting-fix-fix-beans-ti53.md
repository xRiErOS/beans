...
- **REPO-SPLIT (kritisch):** Code-Arbeit + git-Commits laufen im **Worktree** `/Users/erik/dev/lean-stack/beans-src-worktrees/fix-ti53` (Branch `fix/beans-ti53-roadmap-nested-hierarchy`). bean-State (dieses Epic + T1-T6 + Bug beans-ti53) liegt im **Main-Clone** `.beans/`. Jeder `beans`-Befehl MUSS `--beans-path /Users/erik/dev/lean-stack/beans-src/.beans` tragen — sonst greift das Worktree-lokale `.beans/` (kennt diese beans NICHT). Grund: tracking-beans dürfen nie in den Upstream-PR-Diff zu hmans/beans.
